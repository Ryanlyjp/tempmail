...
func main() {
	...
	favoriteGroupH := handler.NewFavoriteGroupHandler(db)
	...
	api.Use(middleware.RateLimit(rdb, cfg.RateLimit, cfg.RateWindow))
	{
		...
		api.DELETE("/mailboxes/batch", favoriteGroupH.BatchDelete)
		api.PUT("/mailboxes/batch/favorite-group", favoriteGroupH.BatchMove)
		api.DELETE("/mailboxes/:id", mailboxH.Delete)
		api.PUT("/mailboxes/:id/favorite", mailboxH.Favorite)
		api.PUT("/mailboxes/:id/favorite-group", favoriteGroupH.MoveMailbox)
		api.PUT("/mailboxes/:id/forward", mailboxH.Forward)
		api.GET("/otp-shares", otpShareH.List)
		...
		api.GET("/favorite-groups", favoriteGroupH.List)
		api.POST("/favorite-groups", favoriteGroupH.Create)
		api.PUT("/favorite-groups/:id", favoriteGroupH.Rename)
		api.PUT("/favorite-groups/:id/select", favoriteGroupH.Select)
		api.PUT("/favorite-groups/:id/reorder", favoriteGroupH.Reorder)
		api.DELETE("/favorite-groups/:id", favoriteGroupH.Delete)

		// 邮件管理
		api.GET("/mailboxes/:id/emails", emailH.List)
		api.GET("/mailboxes/:id/emails/:email_id", emailH.Get)
		api.GET("/mailboxes/:id/emails/:email_id/otp", emailH.ExtractOTP)
